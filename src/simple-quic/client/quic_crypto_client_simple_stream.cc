...
#include "client/quic_client_simple_session_base.h"
...
QuicCryptoClientSimpleStreamBase::QuicCryptoClientSimpleStreamBase(
    QuicClientSimpleSessionBase* session)
    : QuicCryptoStream(session) {}

QuicCryptoClientSimpleStream::ChannelIDSourceCallbackImpl::
    ChannelIDSourceCallbackImpl(QuicCryptoClientSimpleStream* stream)
    : stream_(stream) {}

QuicCryptoClientSimpleStream::ChannelIDSourceCallbackImpl::
    ~ChannelIDSourceCallbackImpl() {}

void QuicCryptoClientSimpleStream::ChannelIDSourceCallbackImpl::Run(
...
void QuicCryptoClientSimpleStream::ChannelIDSourceCallbackImpl::Cancel() {
  stream_ = nullptr;
}

QuicCryptoClientSimpleStream::ProofVerifierCallbackImpl::ProofVerifierCallbackImpl(
    QuicCryptoClientSimpleStream* stream)
    : stream_(stream) {}

QuicCryptoClientSimpleStream::ProofVerifierCallbackImpl::
    ~ProofVerifierCallbackImpl() {}

void QuicCryptoClientSimpleStream::ProofVerifierCallbackImpl::Run(
    bool ok,
...
void QuicCryptoClientSimpleStream::ProofVerifierCallbackImpl::Cancel() {
  stream_ = nullptr;
}

QuicCryptoClientSimpleStream::QuicCryptoClientSimpleStream(
    const QuicServerId& server_id,
    QuicClientSimpleSessionBase* session,
    ProofVerifyContext* verify_context,
    QuicCryptoClientConfig* crypto_config)
    : QuicCryptoClientSimpleStreamBase(session),
...
void QuicCryptoClientSimpleStream::OnHandshakeMessage(
    const CryptoHandshakeMessage& message) {
  QuicCryptoClientSimpleStreamBase::OnHandshakeMessage(message);
...
void QuicCryptoClientSimpleStream::CryptoConnect() {
  next_state_ = STATE_INITIALIZE;
  DoHandshakeLoop(nullptr);
}

int QuicCryptoClientSimpleStream::num_sent_client_hellos() const {
  return num_client_hellos_;
}

// Used in Chromium, but not in the server.
bool QuicCryptoClientSimpleStream::WasChannelIDSent() const {
  return channel_id_sent_;
}

bool QuicCryptoClientSimpleStream::WasChannelIDSourceCallbackRun() const {
  return channel_id_source_callback_run_;
}

void QuicCryptoClientSimpleStream::HandleServerConfigUpdateMessage(
...
void QuicCryptoClientSimpleStream::DoHandshakeLoop(const CryptoHandshakeMessage* in) {
  QuicCryptoClientConfig::CachedState* cached =
      crypto_config_->LookupOrCreate(server_id_);
...
void QuicCryptoClientSimpleStream::DoInitialize(
    QuicCryptoClientConfig::CachedState* cached) {
...
void QuicCryptoClientSimpleStream::DoSendCHLO(
    QuicCryptoClientConfig::CachedState* cached) {
...
void QuicCryptoClientSimpleStream::DoReceiveREJ(
...
QuicAsyncStatus QuicCryptoClientSimpleStream::DoVerifyProof(
    QuicCryptoClientConfig::CachedState* cached) {
...
void QuicCryptoClientSimpleStream::DoVerifyProofComplete(
    QuicCryptoClientConfig::CachedState* cached) {
  if (!verify_ok_) {
...
QuicAsyncStatus QuicCryptoClientSimpleStream::DoGetChannelID(
    QuicCryptoClientConfig::CachedState* cached) {
...
void QuicCryptoClientSimpleStream::DoGetChannelIDComplete() {
...
void QuicCryptoClientSimpleStream::DoReceiveSHLO(
    const CryptoHandshakeMessage* in,
    QuicCryptoClientConfig::CachedState* cached) {
  next_state_ = STATE_NONE;
...
void QuicCryptoClientSimpleStream::DoInitializeServerConfigUpdate(
    QuicCryptoClientConfig::CachedState* cached) {
...
void QuicCryptoClientSimpleStream::SetCachedProofValid(
    QuicCryptoClientConfig::CachedState* cached) {
  cached->SetProofValid();
  client_session()->OnProofValid(*cached);
}

bool QuicCryptoClientSimpleStream::RequiresChannelID(
    QuicCryptoClientConfig::CachedState* cached) {
...
QuicClientSimpleSessionBase* QuicCryptoClientSimpleStream::client_session() {
  return reinterpret_cast<QuicClientSimpleSessionBase*>(session());
}
