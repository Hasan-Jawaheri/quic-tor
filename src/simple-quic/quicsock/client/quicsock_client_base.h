...
class QuicSockClientBase {
 public:
  ...
  QuicSockClientBase(const QuicServerId& server_id,
  ...
  // Returns a newly created QuicSockStream, owned by the
  // QuicSockClient.
  QuicSockStream* CreateReliableClientStream();
  ...
  QuicSockClientSession* session() { return session_.get(); }
 ...
 protected:
  virtual QuicSockClientSession* CreateQuicSockClientSession(
 ...
 private:
  ...
  scoped_ptr<QuicSockClientSession> session_;
  ...
  DISALLOW_COPY_AND_ASSIGN(QuicSockClientBase);
};
