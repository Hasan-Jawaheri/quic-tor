...
#include "quicsock/server/quicsock_time_wait_list_manager.h"
...
// An alarm that informs the QuicSockDispatcher to delete old sessions.
class DeleteSessionsAlarm : public QuicAlarm::Delegate {
 public:
  explicit DeleteSessionsAlarm(QuicSockDispatcher* dispatcher)
 ...
 private:
  // Not owned.
  QuicSockDispatcher* dispatcher_;
...
class QuicSockDispatcher::QuicFramerVisitor : public QuicFramerVisitorInterface {
 public:
  explicit QuicFramerVisitor(QuicSockDispatcher* dispatcher)
  ...
  void OnPublicResetPacket(const QuicPublicResetPacket& /*packet*/) override {
    DVLOG(1) << "OnPublicResetPacket";
  }

  void OnVersionNegotiationPacket(
      const QuicVersionNegotiationPacket& /*packet*/) override {
    DVLOG(1) << "OnVersionNegotiationPacket";
  }

  void OnDecryptedPacket(EncryptionLevel level) override {
    DVLOG(1) << "OnDecryptedPacket";
  }

  bool OnPacketHeader(const QuicPacketHeader& /*header*/) override {
    DVLOG(1) << "OnPacketHeader";
    return true;
  }

  void OnRevivedPacket() override {
    DVLOG(1) << "OnRevivedPacket";
  }

  void OnFecProtectedPayload(StringPiece /*payload*/) override {
    DVLOG(1) << "OnFecProtectedPayload";
  }

  bool OnStreamFrame(const QuicStreamFrame& /*frame*/) override {
    DVLOG(1) << "OnStreamFrame";
    return true;
  }
  ...
  bool OnRstStreamFrame(const QuicRstStreamFrame& /*frame*/) override {
    DVLOG(1) << "OnRstStreamFrame";
    return true;
  }

  bool OnConnectionCloseFrame(
      const QuicConnectionCloseFrame& /*frame*/) override {
    DVLOG(1) << "OnConnectionCloseFrame";
    return true;
  }

  bool OnGoAwayFrame(const QuicGoAwayFrame& /*frame*/) override {
    DVLOG(1) << "OnGoAwayFrame";
    return true;
  }

  bool OnWindowUpdateFrame(const QuicWindowUpdateFrame& /*frame*/) override {
    DVLOG(1) << "OnWindowUpdateFrame";
    return true;
  }
  bool OnBlockedFrame(const QuicBlockedFrame& frame) override {
    DVLOG(1) << "OnBlockedFrame";
    return true;
  }
...
QuicPacketWriter* QuicSockDispatcher::DefaultPacketWriterFactory::Create(
    QuicPacketWriter* writer,
    QuicConnection* connection) {
  return new QuicPerConnectionPacketWriter(writer, connection);
}

QuicSockDispatcher::PacketWriterFactoryAdapter::PacketWriterFactoryAdapter(
    QuicSockDispatcher* dispatcher)
    : dispatcher_(dispatcher) {}

QuicSockDispatcher::PacketWriterFactoryAdapter::~PacketWriterFactoryAdapter() {}

QuicPacketWriter* QuicSockDispatcher::PacketWriterFactoryAdapter::Create(
    QuicConnection* connection) const {
  return dispatcher_->packet_writer_factory_->Create(dispatcher_->writer_.get(),
                                                     connection);
}

QuicSockDispatcher::QuicSockDispatcher(const QuicConfig& config,
...
QuicSockDispatcher::~QuicSockDispatcher() {
  STLDeleteValues(&session_map_);
  STLDeleteElements(&closed_session_list_);
}

void QuicSockDispatcher::InitializeWithWriter(QuicPacketWriter* writer) {
  DCHECK(writer_ == nullptr);
  writer_.reset(writer);
  time_wait_list_manager_.reset(CreateQuicSockTimeWaitListManager());
}

void QuicSockDispatcher::ProcessPacket(const IPEndPoint& server_address,
...
bool QuicSockDispatcher::OnUnauthenticatedPublicHeader(
...
void QuicSockDispatcher::OnUnauthenticatedHeader(const QuicPacketHeader& header) {
  ...
  switch (fate) {
    case kFateProcess: {
      ...
      QuicSockServerSessionBase* session =
          CreateQuicSockSession(connection_id, current_client_address_);
...
QuicSockDispatcher::QuicPacketFate QuicSockDispatcher::ValidityChecks(
...
void QuicSockDispatcher::CleanUpSession(SessionMap::iterator it,
...
void QuicSockDispatcher::DeleteSessions() {
  STLDeleteElements(&closed_session_list_);
}

void QuicSockDispatcher::OnCanWrite() {
...
bool QuicSockDispatcher::HasPendingWrites() const {
  return !write_blocked_list_.empty();
}

void QuicSockDispatcher::Shutdown() {
  while (!session_map_.empty()) {
    QuicSockServerSessionBase* session = session_map_.begin()->second;
...
void QuicSockDispatcher::OnConnectionClosed(QuicConnectionId connection_id,
                                        QuicErrorCode error) {
  SessionMap::iterator it = session_map_.find(connection_id);
  if (it == session_map_.end()) {
...
void QuicSockDispatcher::OnWriteBlocked(
    QuicBlockedWriterInterface* blocked_writer) {
  if (!writer_->IsWriteBlocked()) {
    LOG(DFATAL) << "QuicSockDispatcher::OnWriteBlocked called when the writer is "
                   "not blocked.";
...
void QuicSockDispatcher::OnConnectionAddedToTimeWaitList(
    QuicConnectionId connection_id) {
  DVLOG(1) << "Connection " << connection_id << " added to time wait list.";
}

void QuicSockDispatcher::OnConnectionRemovedFromTimeWaitList(
    QuicConnectionId connection_id) {
  DVLOG(1) << "Connection " << connection_id << " removed from time wait list.";
}

QuicSockServerSessionBase* QuicSockDispatcher::CreateQuicSockSession(
    QuicConnectionId connection_id,
    const IPEndPoint& client_address) {
  ...
  QuicSockServerSessionBase* session =
      new QuicSockServerSession(config_, connection, this, crypto_config_,
      quicsock_observer_.get());
  session->Initialize();
  return session;
}

QuicSockTimeWaitListManager* QuicSockDispatcher::CreateQuicSockTimeWaitListManager() {
  // TODO(rjshade): The QuicSockTimeWaitListManager should take ownership of the
  // per-connection packet writer.
  time_wait_list_writer_.reset(
      packet_writer_factory_->Create(writer_.get(), nullptr));
  return new QuicSockTimeWaitListManager(time_wait_list_writer_.get(), this,
                                     helper_.get());
}

bool QuicSockDispatcher::HandlePacketForTimeWait(
    const QuicPacketPublicHeader& header) {
  if (header.reset_flag) {
...
void QuicSockDispatcher::SetLastError(QuicErrorCode error) {
  last_error_ = error;
}
