...
// A very simple alarm that just informs the QuicSockTimeWaitListManager to clean
// up old connection_ids. This alarm should be cancelled  and deleted before
// the QuicSockTimeWaitListManager is deleted.
class ConnectionIdCleanUpAlarm : public QuicAlarm::Delegate {
 public:
  explicit ConnectionIdCleanUpAlarm(
      QuicSockTimeWaitListManager* time_wait_list_manager)
 ...
 private:
  // Not owned.
  QuicSockTimeWaitListManager* time_wait_list_manager_;
...
class QuicSockTimeWaitListManager::QueuedPacket {
 public:
...
QuicSockTimeWaitListManager::QuicSockTimeWaitListManager(
...
void QuicSockTimeWaitListManager::AddConnectionIdToTimeWait(
...
bool QuicSockTimeWaitListManager::IsConnectionIdInTimeWait(
    QuicConnectionId connection_id) const {
  return ContainsKey(connection_id_map_, connection_id);
}

QuicVersion QuicSockTimeWaitListManager::GetQuicVersionFromConnectionId(
    QuicConnectionId connection_id) {
  ConnectionIdMap::iterator it = connection_id_map_.find(connection_id);
...
void QuicSockTimeWaitListManager::OnCanWrite() {
...
void QuicSockTimeWaitListManager::ProcessPacket(
...
// client.
bool QuicSockTimeWaitListManager::ShouldSendResponse(int received_packet_count) {
  return (received_packet_count & (received_packet_count - 1)) == 0;
}

void QuicSockTimeWaitListManager::SendPublicReset(
    const IPEndPoint& server_address,
    const IPEndPoint& client_address,
    QuicConnectionId connection_id,
...
QuicEncryptedPacket* QuicSockTimeWaitListManager::BuildPublicReset(
...
// owner of the packet.
void QuicSockTimeWaitListManager::SendOrQueuePacket(QueuedPacket* packet) {
...
bool QuicSockTimeWaitListManager::WriteToWire(QueuedPacket* queued_packet) {
...
void QuicSockTimeWaitListManager::SetConnectionIdCleanUpAlarm() {
  connection_id_clean_up_alarm_->Cancel();
...
bool QuicSockTimeWaitListManager::MaybeExpireOldestConnection(
...
void QuicSockTimeWaitListManager::CleanUpOldConnectionIds() {
  QuicTime now = clock_->ApproximateNow();
...
void QuicSockTimeWaitListManager::TrimTimeWaitListIfNeeded() {
...
QuicSockTimeWaitListManager::ConnectionIdData::ConnectionIdData(
    int num_packets_,
