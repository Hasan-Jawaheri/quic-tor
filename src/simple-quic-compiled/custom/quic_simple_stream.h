...
#ifndef NET_QUIC_QUIC_SIMPLE_STREAM_H_
#define NET_QUIC_QUIC_SIMPLE_STREAM_H_
...
class NET_EXPORT_PRIVATE QuicSimpleStream : public ReliableQuicStream {
 public:
  ...
  class NET_EXPORT_PRIVATE Visitor {
   public:
    ...
    virtual void OnClose(QuicSimpleStream* stream) = 0;
  ...
  QuicSimpleStream(QuicStreamId id, QuicSimpleSession* session);
  ~QuicSimpleStream() override;
  ...
  virtual ssize_t Writev(const struct iovec* iov, size_t iov_len, bool is_fin);
  virtual ssize_t Readv(const struct iovec* iov);

 private:
  friend class QuicStreamUtils;

  QuicSimpleSession* quic_session_;
  ...
  DISALLOW_COPY_AND_ASSIGN(QuicSimpleStream);
};

}  // namespace net

#endif  // NET_QUIC_QUIC_SIMPLE_STREAM_H_
